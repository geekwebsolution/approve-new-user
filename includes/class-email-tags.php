 ...
 * Add an email tag
 */
function anu_add_email_tag($tag, $description, $func, $context)
{
    $plugin_instance = anu_approve_new_user();
 ...
 * Remove an email tag
 */
function anu_remove_email_tag($tag)
{
    $plugin_instance = anu_approve_new_user();
 ...
 * Check if $tag is a registered email tag
 */
function anu_email_tag_exists( $tag ) {
	$plugin_instance = anu_approve_new_user();
 ...
 */
function anu_get_emails_tags_list( $context = 'email' ) {
	// The list
	$list = '';

	// Get all tags
	$email_tags = anu_get_email_tags();
 ...
 * Search content for email tags and filter email tags through their hooks
 */
function anu_do_email_tags( $content, $attributes ) {

	$attributes = apply_filters('anu_email_tags_attributes', $attributes);
 ...
 * Load email tags
 */
function anu_load_email_tags() {
	do_action( 'anu_add_email_tags' );
}
add_action( 'init', 'anu_load_email_tags', -999 );

/**
 * Add default anu email template tags
 */
function anu_setup_email_tags() {
 ...
 */
function anu_email_tag_user_email( $attributes ) {
 ...
 * Your site name
 */
function anu_email_tag_sitename( $attributes ) {
 ...
 * Your site URL
 */
function anu_email_tag_siteurl( $attributes ) {
	return home_url();
}
 ...
 * Your site URL
 */
function anu_email_tag_adminurl( $attributes ) {
 ...
 * Your site URL
 */
function anu_email_tag_loginurl( $attributes ) {
	return wp_login_url();
}
 ...
 */
function anu_email_tag_password( $attributes ) {
	$user = $attributes['user'];

	if ( anu_approve_new_user()->do_password_reset( $user->ID ) ) {
		...
		update_user_meta( $user->ID, 'anu_user_approve_password_reset', time() );
		/* translators: %s: search term */
 ...
 */
function anu_email_tag_reset_password_url( $attributes ) {
    global $wpdb;
